...
func (d *Deserializer) RegistrationRequest(registrationRequest []byte) (*message.RegistrationRequest, error) {
	if len(registrationRequest) != d.conf.Sizes.RegistrationRequest {
...
func (d *Deserializer) RegistrationResponse(registrationResponse []byte) (*message.RegistrationResponse, error) {
	if len(registrationResponse) != d.conf.Sizes.RegistrationResponse {
	...
	evaluatedMessage, err := DeserializeElement(d.conf.OPRF.Group(),
		registrationResponse[:d.conf.Sizes.OPRFElement])
	if err != nil {
		return nil, ErrRegistrationResponse.Join(internal.ErrInvalidEvaluatedMessage, err)
	}

	offset := d.conf.Sizes.OPRFElement
	pksBytes := registrationResponse[offset : offset+d.conf.Sizes.AKEElement]
...
func (d *Deserializer) RegistrationRecord(record []byte) (*message.RegistrationRecord, error) {
	if len(record) != d.conf.Sizes.RegistrationRecord {
		return nil, ErrRegistrationRecord.Join(internal.ErrInvalidMessageLength)
	}

	pk := record[:d.conf.Sizes.AKEElement]
	maskingKey := record[d.conf.Sizes.AKEElement : d.conf.Sizes.AKEElement+d.conf.Sizes.Hash]
	env := record[d.conf.Sizes.AKEElement+d.conf.Sizes.Hash:]
...
func (d *Deserializer) KE1(ke1 []byte) (*message.KE1, error) {
	if len(ke1) != d.conf.Sizes.KE1 {
		return nil, ErrKE1.Join(internal.ErrInvalidMessageLength)
	}

	request, err := d.deserializeCredentialRequest(ke1[:d.conf.Sizes.CredentialRequest])
	if err != nil {
		return nil, ErrKE1.Join(err)
	}

	nonceU := ke1[d.conf.Sizes.OPRFElement : d.conf.Sizes.OPRFElement+d.conf.Sizes.Nonce]

	if isAllZeros(nonceU) {
		return nil, ErrKE1.Join(internal.ErrMissingNonce, internal.ErrSliceIsAllZeros)
	}

	offset := d.conf.Sizes.OPRFElement + d.conf.Sizes.Nonce

	epku, err := DeserializeElement(d.conf.Group, ke1[offset:offset+d.conf.Sizes.AKEElement])
	if err != nil {
...
func (d *Deserializer) KE2(ke2 []byte) (*message.KE2, error) {
	...
	if len(ke2) != d.conf.Sizes.KE2 {
		return nil, ErrKE2.Join(internal.ErrInvalidMessageLength)
	}

	cresp, err := d.deserializeCredentialResponse(ke2, d.conf.Sizes.CredentialResponse)
	if err != nil {
	...
	nonceS := ke2[d.conf.Sizes.CredentialResponse : d.conf.Sizes.CredentialResponse+d.conf.Sizes.Nonce]
	offset := d.conf.Sizes.CredentialResponse + d.conf.Sizes.Nonce
	epk := ke2[offset : offset+d.conf.Sizes.AKEElement]
	offset += d.conf.Sizes.AKEElement
	mac := ke2[offset:]
...
func (d *Deserializer) deserializeCredentialResponse(
	...
	evaluatedMessage, err := DeserializeElement(d.conf.OPRF.Group(), input[:d.conf.Sizes.OPRFElement])
	if err != nil {
		return nil, errors.Join(internal.ErrInvalidEvaluatedMessage, err)
	}

	return message.NewCredentialResponse(evaluatedMessage,
		input[d.conf.Sizes.OPRFElement:d.conf.Sizes.OPRFElement+d.conf.Sizes.Nonce],
		input[d.conf.Sizes.OPRFElement+d.conf.Sizes.Nonce:maxResponseLength]), nil
}
