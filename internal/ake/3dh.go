...
func initTranscript(h *internal.Hash, idc, ids []byte, ke1 *message.KE1, ke2 *message.KE2) {
	cp := encoding.EncodeVectorLen(idc, 2)
	sp := encoding.EncodeVectorLen(ids, 2)
	h.Write(utils.Concatenate(0, []byte(internal.VersionTag), cp, ke1.Serialize(), sp, ke2.CredentialResponse.Serialize(), ke2.NonceS, ke2.EpkS))
}
...
func core3DH(s selector, p *internal.Parameters, k *coreKeys, idu, ids []byte,
	...
	initTranscript(p.Hash, idu, ids, ke1, ke2)
