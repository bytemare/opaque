...
func TestServerKeyMaterial_DecodeHexCases(t *testing.T) {
	...
	if _, err := conf.DecodeServerKeyMaterialHex(""); !errors.Is(err, internal.ErrDecodingEmptyHex) {
		t.Fatalf("expected empty hex error, got %v", err)
	}

	if _, err := conf.DecodeServerKeyMaterialHex("zz"); err == nil {
		t.Fatal("expected invalid hex error")
	}

	encoded := hex.EncodeToString(skm.Encode())
	decoded, err := conf.DecodeServerKeyMaterialHex(encoded)
	if err != nil {
		t.Fatalf("unexpected error decoding valid hex: %v", err)
	}
	if decoded.PrivateKey == nil || decoded.PublicKeyBytes == nil {
		t.Fatal("expected decoded key material")
	}
}
...
func TestServerKeyMaterial_Flush(t *testing.T) {
	...
	skm := &opaque.ServerKeyMaterial{
		Identity:       append([]byte(nil), serverIdentity...),
		PrivateKey:     sk,
		PublicKeyBytes: append([]byte(nil), pk.Encode()...),
		OPRFGlobalSeed: append([]byte(nil), seed...),
	}

	skm.Flush()

	if skm.PrivateKey != nil {
		t.Fatal("expected private key to be cleared")
	}
	if skm.PublicKeyBytes != nil {
		t.Fatal("expected public key bytes to be cleared")
	}
	if skm.OPRFGlobalSeed != nil {
		t.Fatal("expected OPRF seed to be cleared")
	}
	if skm.Identity != nil {
		t.Fatal("expected identity to be cleared")
	}
}
